<?php foreach($hiendh as $vl){
    extract($vl);
}
if(isset($_POST['trang_thai_don'])){
  $trang_thai_don=$_POST['trang_thai_don'];
  mysqli_query($conn,"UPDATE don_hang SET trang_thai_don='$trang_thai_don' where ma_don_hang='$ma_don_hang'");
  header('location:admin.php?act=suadh');
...
?>  
  ...
  <footer class="pt-6 pb-10 footer-dashboard mt-auto">
    <div class="row">
      ...
      <div class="col-sm-6 text-sm-end text-center"> </div>
    </div>
  </footer>
</main>
<?php if(($trang_thai_don !=2)&&($trang_thai_don !=3)&&($trang_thai_don ==0)){?>
<div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form action="admin.php?act=suadh&ma_don_hang=<?php echo $ma_don_hang; ?>" enctype="multipart/form-data" method="post">
                <div class="card-body">
                  <div class="form-group"></div>
                    <label for="exampleInputEmail1">Trạng thái đơn</label>
                    <!-- <input type="text" value="<?php echo  $trang_thai_don  ?>" name="trang_thai_don" class="form-control" id="exampleInputEmail1"> -->
                    <select id="orderStatus" name="trang_thai_don">
        ...
        <!-- <option value="3">   Đơn hàng đã bị hủy</option> -->
                ...
                <input type="submit" name="suadh"class="btn btn-primary" value="Cập nhật "></input>
                </div>
              </form>
            </div>
            <?php } elseif(($trang_thai_don !=2)&&($trang_thai_don !=3)&&($trang_thai_don ==4)){?>
<div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Sửa đơn hàng</h3>
              </div>
